...
public class Vitamio {
	 ...
	 * <p/>
	 * This method will use {@link #isInitialized(Context)} to check if Vitamio
	...
	public static boolean initialize(Context ctx) {
		return isInitialized(ctx) || extractLibs(ctx);
	}
...
//  }
	...
	public static boolean isInitialized(Context ctx) {
		...
		vitamioLibraryPath = ContextUtils.getDataDir(ctx) + "doVitamioSo/";
	...
	private static boolean extractLibs(Context ctx) {
		...
		if (lock.exists())
			lock.delete();
		String libPath = ContextUtils.getDataDir(ctx) + "lib/libarm.so";
