 ...
 */
public class MediaController extends FrameLayout implements DoIModuleTypeID {
...
//	private String mTitle;
	 ...
	 */
	public void setAnchorView(View view, DoUIModule model, boolean isFullScreen) {
		mAnchor = view;
		this.isFullScreen = isFullScreen;
		if (!mFromXml) {
			removeAllViews();
			mRoot = makeControllerView(model, isFullScreen);
	 ...
	 */
	protected View makeControllerView(DoUIModule model, boolean isFullScreen) {
		int mediacontroller_id = DoResourcesHelper.getIdentifier("vitamio_mediacontroller", "layout", this);
		FrameLayout mView = (FrameLayout) ((LayoutInflater) mContext.getSystemService(Context.LAYOUT_INFLATER_SERVICE)).inflate(mediacontroller_id, this);
		if (!isFullScreen) {
			View mChildView = mView.getChildAt(0);
			FrameLayout.LayoutParams lp = (LayoutParams) mChildView.getLayoutParams();
			int[] position = new int[2];
			View parentView = (View) model.getCurrentUIModuleView();
			parentView.getLocationOnScreen(position);

			lp.leftMargin = position[0];
			lp.width = (int) model.getRealWidth();

			mRootTop = (int) (model.getRealHeight()) - lp.height;
		}
		return mView;
	}

	private void initControllerView(View v) {
		int mediacontroller_play_pause_id = DoResourcesHelper.getIdentifier("mediacontroller_play_pause", "id", this);
		mPauseButton = (ImageButton) v.findViewById(mediacontroller_play_pause_id);
		if (mPauseButton != null) {
			mPauseButton.requestFocus();
			mPauseButton.setOnClickListener(mPauseListener);
		}

		int mediacontroller_fullscreen_id = DoResourcesHelper.getIdentifier("mediacontroller_fullscreen", "id", this);
		mFullScreenButton = (ImageButton) v.findViewById(mediacontroller_fullscreen_id);
		if (mFullScreenButton != null) {
			mFullScreenButton.requestFocus();
			mFullScreenButton.setOnClickListener(mFullScreenListener);
		}

		int mediacontroller_seekbar_id = DoResourcesHelper.getIdentifier("mediacontroller_seekbar", "id", this);
		mProgress = (SeekBar) v.findViewById(mediacontroller_seekbar_id);
		if (mProgress != null) {
		...
		int mediacontroller_time_total_id = DoResourcesHelper.getIdentifier("mediacontroller_time_total", "id", this);
		mEndTime = (TextView) v.findViewById(mediacontroller_time_total_id);
		int mediacontroller_time_current_id = DoResourcesHelper.getIdentifier("mediacontroller_time_current", "id", this);
		mCurrentTime = (TextView) v.findViewById(mediacontroller_time_current_id);
//		int mediacontroller_file_name_id = DoResourcesHelper.getIdentifier("mediacontroller_file_name", "id", this);
//    mFileName = (TextView) v.findViewById(mediacontroller_file_name_id);
//    if (mFileName != null)
...
//      mFileName.setText(mTitle);
		...
		if (!mShowing && mAnchor != null && mAnchor.getWindowToken() != null) {
			...
			if (mFromXml) {
				...
				if (isFullScreen) {
					...
					mWindow.showAtLocation(mAnchor, Gravity.NO_GRAVITY, location[0], mRootTop + location[1]);
				}
			}
			mShowing = true;
	...
	private void updatePausePlay() {
		...
		if (mPlayer.isPlaying())
			mPauseButton.setImageResource(DoResourcesHelper.getIdentifier("vitamio_mediacontroller_pause", "drawable", this));
		else
			mPauseButton.setImageResource(DoResourcesHelper.getIdentifier("vitamio_mediacontroller_play", "drawable", this));
	}
	...
	private void doFullScreen(View view) {
		if (mFullScreenClickListener != null) {
			mFullScreenClickListener.onFullScreenClick(view);
		}
	}
