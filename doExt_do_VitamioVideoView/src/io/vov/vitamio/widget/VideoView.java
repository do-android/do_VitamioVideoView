...
public class VideoView extends SurfaceView implements MediaController.MediaPlayerControl {
	...
	private OnErrorListener mErrorListener = new OnErrorListener() {
		public boolean onError(MediaPlayer mp, int framework_err, int impl_err) {
			...
			if (getWindowToken() != null) {
				int message = framework_err == MediaPlayer.MEDIA_ERROR_NOT_VALID_FOR_PROGRESSIVE_PLAYBACK ? getResources().getIdentifier("VideoView_error_text_invalid_progressive_playback", "string",
						mContext.getPackageName()) : getResources().getIdentifier("VideoView_error_text_unknown", "string", mContext.getPackageName());

				new AlertDialog.Builder(mContext).setTitle(getResources().getIdentifier("VideoView_error_title", "string", mContext.getPackageName())).setMessage(message)
						.setPositiveButton(getResources().getIdentifier("VideoView_error_button", "string", mContext.getPackageName()), new DialogInterface.OnClickListener() {
	...
	public VideoView(Context context, DoUIModule model, boolean isFullScreen) {
		super(context);
		this.model = model;
	...
	public void setVideoLayout(int layout, float aspectRatio) {
		...
		int windowWidth = lp.width, windowHeight = lp.height;
		if (isFullScreen) {
			...
			if (model != null) {
				windowWidth = (int) model.getRealWidth();
				windowHeight = (int) model.getRealHeight();
			}
		}
		...
		Log.d("VIDEO: %dx%dx%f, Surface: %dx%d, LP: %dx%d, Window: %dx%dx%f", mVideoWidth, mVideoHeight, mVideoAspectRatio, mSurfaceWidth, mSurfaceHeight, lp.width, lp.height, windowWidth,
...
//	}
	...
	public void setVideoURI(String playUrl) throws Exception {
		if (DoIOHelper.getHttpUrlPath(playUrl) != null) {
			if (this.model.getPropertyValue("isCache") == "true") {
				String cacheDir = "data://vitamioCache";
				String cacheFileName = playUrl.substring(playUrl.lastIndexOf("/") + 1);

				File file = new File(DoIOHelper.getLocalFileFullPath(model.getCurrentPage().getCurrentApp(), cacheDir));
				if (!file.exists() && !file.isDirectory()) {
					file.mkdir();
				}
				String cachePath = DoIOHelper.getLocalFileFullPath(model.getCurrentPage().getCurrentApp(), cacheDir + "/" + cacheFileName);
				if (DoIOHelper.existFile(cachePath)) {
					setVideoURI(Uri.parse(cachePath), null);
				} else {
					Uri uri = Uri.parse("cache:" + cachePath + ":" + playUrl);
					setVideoURI(uri, null);
				}
			} else {
				setVideoURI(Uri.parse(playUrl), null);
			}
		} else {
			String localPath = DoIOHelper.getLocalFileFullPath(this.model.getCurrentPage().getCurrentApp(), playUrl);
			setVideoURI(Uri.parse(localPath), null);
		}
	}
	...
	private void attachMediaController() {
		if (mMediaPlayer != null && mMediaController != null) {
			...
			mMediaController.setAnchorView(anchorView, model, isFullScreen);
	...
	public void resume() {
		if (mSurfaceHolder == null && mCurrentState == STATE_SUSPEND) {
			...
			openVideo();
		} else if (mCurrentState == STATE_PAUSED) { // 继续播放
			if (mMediaPlayer != null) {
				start();
